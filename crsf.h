...
struct crsfParameter {
    ...
    uint8_t type;                     // 0x09 = Select, 0x0B = Folder, etc.
    uint8_t currentVal;               // Currently active option index
    char label[CRSF_MAX_STRING_LEN];  // Human-readable setting name (e.g., "Model Match")
    
    // Extracted selectable strings (e.g., {"Off", "On"})
    crsfOption choices[10];           // Safe allocation pool for up to 10 sub-choices (packet rate currently has 8)
